...
|---|---|---|
| `-webkit-line-clamp` CSS text clamping | Confirmed working in the extension preview browser (Chromium/Blink). The combination `display:-webkit-box; -webkit-box-orient:vertical; -webkit-line-clamp:N; overflow:hidden` truncates text to N lines with an ellipsis. This property has been in Blink/WebKit since Chrome 14 (2011); Tableau Desktop uses QtWebEngine (Chromium) and Tableau Cloud uses a Chromium-based renderer — both well past Chrome 14. Use the three-property block together; `overflow:hidden` alone does not clamp. `margin-top:auto` (flex bottom-pin) on the clamped element works alongside `-webkit-box` display because the parent flex context positions the element independently of its internal display. Standard `line-clamp` (no prefix) is not yet used — keep the webkit prefix. | `[observed]` preview browser 2026-06-09 b_tabext_nav; inferred Tableau embedded Chromium from broad Blink history |
...
| `getSummaryDataAsync` returns **only the rows present in the worksheet's current view/filters** | It is not a query against the data source — it returns what the worksheet shows. A raw-values worksheet scoped/filtered to a single `indicator_id` (e.g. `scorer_rawvals` showing only `eqr`) yields **only that indicator's rows** to the extension (20 rows = 5 brands × 4 periods). The extension cannot widen this from JS; the **worksheet must place/show all indicators' rows** (remove the indicator filter, or build the sheet at the right grain). Symptom: every other indicator falls back to its `/5` score-mirror because it has no raw rows. | `[tested]` 2026-06-16 index_examples.html b_tabext_nav |
| Column-shape worksheet auto-detect is **not reliable when ≥2 sheets share the shape** — name the intended sheet | Multiple worksheets on one dashboard can share `Brand Name Upper` + `indicator_id` + a value/period shape. A "first sheet that matches" heuristic — even one that prefers a period-bearing candidate over the score sheet — can still pick the **wrong raw sheet** when two raw-shaped period-bearing sheets exist. Observed: the heuristic picked `scorer_rawvals` (period-bearing but scoped to `eqr` only) over the intended `raw_values_filtered`. **Resolution: set the sheet name explicitly** (`CONFIG.worksheet='raw_values_filtered'`), exactly as the `index.html` donor does; keep the period-bearing heuristic only as a fallback when the named sheet is absent. | `[tested]` 2026-06-16 index_examples.html b_tabext_nav |
...
| YouTube `<iframe>` embed inside a Network Enabled extension | Works in Tableau Cloud. **Required:** use `https://www.youtube.com/embed/<id>` (not `youtube-nocookie.com`) and append `?origin=<extension-host-origin>` (e.g. `?origin=https://bengtcj.github.io`). Without the `origin` param, or with the nocookie domain, Tableau's sandbox returns YouTube Error 153 "Video player configuration error". The `allow` attribute (`accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share`) and `allowfullscreen` are required as on any embed. | `[tested]` 2026-06-12 examples.html b_tabext_nav |
| Hotlinked external OG images (`<img src="https://...">`) | External images load fine — Tableau Cloud's CSP `img-src` does not block third-party HTTPS image hosts. Confirmed with `s3.mordorintelligence.com` (a public S3 bucket). No proxy needed for standard og:image hotlinking in v1. | `[tested]` 2026-06-12 examples.html b_tabext_nav |
| jsDelivr CDN for the Extensions API library | `https://cdn.jsdelivr.net/gh/tableau/extensions-api/lib/tableau.extensions.1.latest.js` loads and initialises successfully in a Network Enabled extension on Tableau Cloud. All three extensions in this repo now use the CDN path — no local lib copy needed. | `[observed]` 2026-06-09 b_tabext_nav (index.html, solutions_nav.html, examples.html) |
...
| Preview / sample-data mode: how it's triggered and what chart types it can reach | Preview mode (sample data, no Tableau connection) is triggered simply by `typeof tableau === 'undefined'` — i.e. opening the extension URL in a plain browser. CLAUDE.md's "`?chart=`" notation means "open the URL with any query string"; **there is no `?chart=` query-string handler in the code**. The only URL parameter actually parsed is `?subgroup=` (pre-filters the indicator table). In preview mode, `switchToChart(indicatorId)` always sets `CONFIG.chart = resolveChart(indicatorId)`, which picks `METRIC_CHARTS[id][0]` — the registered default. **There is no existing URL or UI mechanism to target a non-default chart type.** The six defaults reachable by table-click: `ban_editorial` (tam/cagr/mcon), `vbar` (eqr/ebl/sstsr), `treemap` (sov/bss), `line-straight` (vom/svt), `scale-figma` (bt/nps/sop/dvtr), `hbar` (cra/ba). All other 21 chart IDs require browser-console override: `CONFIG.chart='<id>'; renderChart(getSampleData('<id>'), CONFIG.clientBrand, null)`. `[observed]` 2026-06-08 b_tabext_nav (code audit) |
...
| `getSummaryDataAsync` can return 0 columns **and** 0 rows when upstream filters match nothing | Observed: a pipeline change caused a different run-ID value to be used, so the worksheet's filter matched 0 rows — and the returned DataTable had 0 columns as well as 0 rows (not just empty data with columns intact). Symptom: `console.warn('Required fields missing. Available: ')` with a blank after the colon. The improved error now logs `cols=0 rows=0` to distinguish this from a field-name mismatch (cols > 0). **Before assuming a code or manifest problem, check upstream filters and pipeline run-ID values.** | `[observed]` 2026-06-09 b_tabext_nav |
...
| Null date-part field returns the string `'%null%'` as `.value`, not JS `null` | For a dimension field (e.g. `Year`) that is null in the underlying data, `DataValue.value` is the **string** `'%null%'` — not JavaScript `null`. `DataValue.formattedValue` returns `'Null'` (capital N). Neither is parseable as a number. Consequence: `parseFloat('%null%')` = `NaN`; a truthiness check on `.value` passes (non-empty string) while `isNaN` catches it. Guard with an explicit null-sentinel check: `if (v === '%null%' \|\| v === 'Null' \|\| v === null) ...` | `[tested]` 2026-06-02 b_tabext_nav |
| A dimension can be null for one indicator but populated for another in the same worksheet | When multiple indicators from different data sources (e.g. survey vs Snowflake) share one worksheet, a date dimension like `Year` may be correctly populated for some indicators and `'%null%'` for others. The field is genuinely absent from one source's date model. `getSummaryDataAsync` returns all rows but the null-Year rows carry no recoverable year. Fix: add a pre-computed calculated field (e.g. `STR(YEAR([date])) + '-Q' + STR(DATEPART('quarter', [date]))`) to the worksheet and read it via `periodField` config — do not try to infer the year from cross-row heuristics. | `[tested]` 2026-06-02 b_tabext_nav (SVT / Snowflake source) |
...
| Native font-size picker is a fixed discrete list — no 13 | Tableau's text formatting offers 10, 11, 12, 14, 15, 16, 18… with no 13. Extension HTML can use any px (the spec's 13px Label); native Tableau text cannot be set to 13 and must snap to the nearest step (14). Match native realizations of the px tokens to the nearest pickable size — don't assume parity with the extension. Whether a custom value can be typed into the size field is version/context-dependent — to verify in our version. | `[observed]` 2026-06-16 user report, b_tabext_nav (native page); custom-entry question open |
