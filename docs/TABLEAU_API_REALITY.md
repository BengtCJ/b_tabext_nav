...
| Disambiguate co-located worksheets by **column shape + period dimension** | Multiple worksheets on one dashboard can share `Brand Name Upper` + `indicator_id`, so a plain "first sheet with brand+indicator+value" auto-detect can pick the **score** sheet instead of the **raw** sheet. The raw sheet is distinguished by also carrying a period dimension (`Year`/`Quarter`/`Month`/period field); the score sheet has none. Preferring the period-bearing candidate correctly selects the raw sheet (confirmed: picked `scorer_rawvals` over the score sheet). | `[tested]` 2026-06-16 index_examples.html b_tabext_nav |
